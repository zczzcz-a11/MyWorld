...
<template>
  <div class="sticky top-0  z-10  bg-slate-50 p-4 dark:bg-black">
    <span
      v-for="item in typeList"
      :key="item.value" class="cursor-pointer rounded-md px-2 opacity-50 transition duration-150 ease-in-out"
  ...
  <div class="mt-4">
    <div v-show="isLoading">
      <div v-for="item in 9" :key="item" class="flex items-center justify-between p-2">
        <div class="flex items-center">
          <n-skeleton width="15px" class="mt-2" type="text" />
          <n-skeleton
            class="ml-2 mt-2" height="64px" width="64px"
            :sharp="false"
          />
        </div>
        <n-skeleton
      ...
      <ul v-show="!isLoading" class="songList">
        ...
        > 
          <div class="flex items-center" style="{ width: '120px' }">
            ...
            <div style="-webkit-transform: translateZ(0);" class="relative ml-4 h-16 w-16 rounded-md">
              <load-img
                loading-height="64px"
          ...
          <p class="flex-30 ml-6 w-xs truncate text-sm">
            {{ item.name }}
            <n-tag
          ...
          <p class="flex-30 ml-2 w-xs text-sm opacity-80">
            <n-ellipsis>{{ formateSongsAuthor(item.artists) }}</n-ellipsis>
          </p>
          <p class="flex-30 ml-2 w-xs flex-1 truncate text-sm opacity-80">
            {{ item.album.name }}
          </p>
          <n-time
            class="mx-2 pl-4 text-left text-sm opacity-80"
